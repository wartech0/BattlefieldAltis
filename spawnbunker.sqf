...
_bunker = "Land_BagBunker_Large_F" createVehicle _spawnpos;
_bunker setPosATL _spawnpos;
...
{ if (_x != _bunker) then { deleteVehicle _x } } forEach nearestObjects [_spawnpos, ["static","Land_Sea_Wall_F"], 10];
...
for "_x" from 1 to _count step 1 do {
	...
	_ranpos = [(getPosATL _bunker select 0) + ((sin _randir) * (1+((random 300)/100))), (getPosATL _bunker select 1) + ((cos _randir) * (1+((random 300)/100))), getPosATL _bunker select 2];
